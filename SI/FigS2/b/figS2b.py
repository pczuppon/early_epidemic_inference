...
for i in range(int(tfin/dt)-1):
    ...
    if (tot[-1] >= k):
        ...
        #mu_adj = (1-pext[0]*np.product(pext[0:(i+1)]*fx[i,0:(i+1)]))/(1-np.product(pext[0:(i+1)]*fx[i,0:(i+1)]))
        k += 1    
...
plt.hist(time, bins=np.arange(0,np.max(time),2),density=True,alpha=0.5,color='C1')
plt.plot(t,hosp_time_dist,color='C1',linewidth=3)
plt.plot(t,hosp_time_dist2,color='black',linewidth=3)
plt.axvline(np.mean(time),color='C1',linewidth=3,linestyle='dotted')
plt.axvline(np.sum(dt*t*hosp_time_dist),color='C1',linewidth=3,linestyle='dashed')
