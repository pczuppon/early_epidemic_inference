...
int RUN(int tfin,int r_ind)
{
    ...
    {
        t = 0;
        ofstream file ("trajectory_tfin_" + to_string(tfin) + "_id_" + to_string(r_ind) + ".txt", ios::app); 
        for (int ind = 0; ind <= length; ind++)
        {
            file << t;
            file << ",";
            file << accumulate(I_work,I_work+ind,0);
            file << "\n";
