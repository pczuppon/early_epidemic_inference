...
// g++ -std=c++11 00_sim_early_dynamics_poisson.cpp `pkg-config --libs gsl` command for compiling
...
gsl_rng * rbin = gsl_rng_alloc (gsl_rng_mt19937);
gsl_rng * rrest = gsl_rng_alloc (gsl_rng_mt19937);
...
int main()
{
    ...
    {
        gsl_rng_set(rbin,r_ind);                    // setting the seed
        gsl_rng_set(rrest,r_ind);
...
int RUN(int r_ind)
{
    ...
    unsigned int gsl_ran_poisson(const gsl_rng * rrest, double mu);  // mu = mean
    ...
    {   
        ...
        int n_hosp = gsl_ran_binomial(rbin, p_hosp, I_work[work_ind]);
        
        // update minimal hospitalization time
        for (int i = 1; i <= n_hosp; i++)
        {
            double hosp_time = t + gsl_ran_gamma(rrest,shape_hosp,scale_hosp);
        ...
        {
            offspring += gsl_ran_poisson(rrest,R0);
        }
        
        // loop over offsprings from individuals that were added at time t
        for (int i = 1; i <= offspring; i++)
        {   
            double inf_time = t+gsl_ran_gamma(rrest,shape_inf,scale_inf);
    ...
    if (return_value ==1)
    {
        ...
        ofstream file2 ("size_time_poisson.txt", ios::app);
        file2 << hosp_min;
        file2 << ",";
